...
import BoarderLine from "../components/borderLine.vue";
import WeatherForecastDay from "../components/weathreForcast.vue"
...
<template>
  <div class="bg-day-night text-white p-10 rounded-lg shadow-lg gap-6 mb-6 relative overflow-hidden">
    <!-- Location & time -->
    ...
    <div class="text-center">
      <img :src="place.current.condition.icon" alt="icon" width="200" class="mx-auto mb-4 md:-mb-10" />
      <h1 class="text-5xl md:text-9xl mb-2 -mr-4">{{ Math.round(place.current.temp_c) }}&deg;</h1>
      <p class="text-lg md:text-2xl">{{ place.current.condition.text }}</p>
    </div>

    <!-- Border line -->
    <hr class="border-white my-6 md:hidden" />
    ...
    <!-- forecast btn -->
    <div class="flex justify-end items-center gap-1 mt-6 md:mt-10">
